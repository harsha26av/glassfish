...
/*
 * Unit test to make sure that AsyncListener#onComplete is called.
 */
public class WebTest {

    private static final String TEST_NAME = "servlet-3.0-async-listener-on-complete";
    ...
    public void doTest(String mode) throws Exception {
        ...
        try {
            conn = getHttpURLConnection("/TestServlet?" + mode);
            conn.disconnect();
    ...
    private HttpURLConnection getHttpURLConnection(String path) throws Exception {
        URL url = Urls.create("http://" + host  + ":" + port +
            contextRoot + path, Urls.HTTP_PROTOCOLS, HostValidator.DENY_COMMON_INFRASTRUCTURE_TARGETS);
